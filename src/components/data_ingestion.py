...
class ChatIngestor:
    def __init__( self,
        ...
        try:
            ...
            self.chroma_base = Path(chroma_base); self.chroma_base.mkdir(parents=True, exist_ok=True)

            self.temp_dir = self._resolve_dir(self.temp_base)
            self.chroma_dir = self._resolve_dir(self.chroma_base)
            ...
            raise CustomException("Initialization error in ChatIngestor", e) 


    def _resolve_dir(self, base: Path):
        if self.use_session:
            d = base / self.session_id # e.g. "chroma_index/abc123"
            d.mkdir(parents=True, exist_ok=True) # creates dir if not exists
            return d
        return base # fallback: "chroma_index/"
    ...
    def build_retriever( self,
        ...
        *,
        ...
        try:
            ...
            ## chroma manager very very important class for the docchat
            fm = ChromaManager(self.chroma_dir, self.model_loader)
            ...
            logger.info("chroma index updated", added=added, index=str(self.chroma_dir))
...
class ChromaManager:
    def __init__(self, index_dir: Path, model_loader: Optional[ModelLoader] = None):
        ...
        if self.meta_path.exists():
            try:
                self._meta = json.loads(self.meta_path.read_text(encoding="utf-8")) # load it if alrady there
            except Exception:
    ...
    def _exists(self)-> bool:
        return (self.index_dir / "index.chroma").exists() and (self.index_dir / "index.pkl").exists()
    ...
    def load_or_create(self,texts:Optional[List[str]]=None, metadatas: Optional[List[dict]] = None):
        ...
        if not texts:
            raise CustomException("No existing chroma index and no data to create one", sys)
        self.vs = Chroma.from_texts(texts=texts, embedding=self.emb, metadatas=metadatas or [])
        self.vs.save_local(str(self.index_dir))
        return self.vs